...
async function sendImage() {
  ...
  const messageContent = '📸 Image via MoltTalk\nhttps://gateway.pinata.cloud/ipfs/' + cid;
  
  // Now send encrypted through MoltTalk
  ...
  const fullCID = await uploadToPinata(payload, PINATA_JWT);
  const cidHash = cidToBytes32(fullCID);
  
  console.log('⛓️  Sending encrypted image reference to Amir on Monad...');
  ...
  await tx.wait();
  console.log('✅ Encrypted image sent to Amir!');
